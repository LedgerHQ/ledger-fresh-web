...
async function getWebWalletStarknetObject(options?: {
  url?: string;
  show?: boolean;
  ...
  const url_account: string =
    options?.url || "https://fresh-web.vercel.app/fresh";
  const icon =
  ...
  const { iframe, modal } = await createModal(
    url_account,
    options?.show || false
  );
